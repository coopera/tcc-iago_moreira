...
Pode-se concluir que essas dúvidas geralmente se enquadravam em alguma das seguintes categorias:

% IM: descobrir como fazer uma lista
1. Como implementar uma determinada funcionalidade.
2. Qual a melhor biblioteca para realizar determinada função.
3. Qual era um bom tutorial que ensinava o uso de alguma biblioteca
4. Como resolver determinado \textit{bug}.

Para todos os casos citados, o fluxo de resolução do problema se dava na maioria dos casos da seguinte forma:

% IM: descobrir como fazer uma lista numerada
1. O desenvolvedor comunicava o problema (para todos os membros ou somente um) no \textit{chat} utilizado pela equipe.
2. Para os casos 1 e 4, os desenvolvedores apontavam um ou mais trechos de código da equipe que implementava corretamente a funcionalidade proposta. Para o caso 2, havia uma discussão a respeito do contexto da aplicação a ser desenvolvida e um se chegava a um consenso sobre qual a melhor biblioteca para resolver o problema mencionado. Para o caso 3, um ou mais links eram disponibilizados.
3. O desenvolvedor que comunicou o problema tentava implementar a funcionalidade utilizando as recomendações oferecidas.
4. Caso novas dúvidas surgissem durante a implementação, os demais membros da equipe tentavam dar recomendações mais precisas ou oferecer outros exemplos e links úteis.
5. O problema era solucionado.
...
% IM: vale a pena deixar essa parte abaixo?
